...
@Component({
  selector: 'app-transaction-listing-report',
  templateUrl: './transaction-listing-report.component.html',
  styleUrls: ['./transaction-listing-report.component.scss']
})
export class TransactionListingReportComponent implements OnInit {
  ...
  outletList: MobileUser[];
  loading = false;
  isTableLoading = false;
  agentsLoading = false;
  form: FormGroup;
  cols: any[];
  customers: RPTTransaction[] = [];
  ...
  findTransactingAgents() {
    let request = new MobileUser; 
    request.acctType = "OUTLET";
    this.outletList = [];
    this.agentsLoading = true;
    this.agentService.findTransactingAgents(request)
      .pipe(first())
      .subscribe(
        response => {
  ...
  findReportData() {
    ...
    this.customers = [];
    this.reportService.findTransactionListingReport(this.form.value)
      .pipe(first())
      .subscribe(
        response => {
  ...
  public exportPdf() {
    ...
    for (const element of this.customers) {
      item = [element.systemDate, element.transDescr, element.amount];
      data.push(item);
    }
    autoTable(doc, {
      head: head,
