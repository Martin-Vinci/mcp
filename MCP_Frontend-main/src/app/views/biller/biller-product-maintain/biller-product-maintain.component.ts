...
@Component({
  selector: 'app-biller-product-maintain',
  templateUrl: './biller-product-maintain.component.html',
  styleUrls: ['./biller-product-maintain.component.scss']
})
export class BillerProductMaintainComponent implements OnInit {
  ...
  createForms() {
    this.form = this.fb.group({
      ...
      billerProdCatId: [null, Validators.required],
      billerId: [this.product.id],
      description: [null],
      billerProdCode: [null, Validators.required],
  ...
  findEntityData() {
    this.f.billerProductId.setValue(this.selectedData.billerProductId);
    this.f.billerProdCatId.setValue(this.selectedData.billerProdCatId);
    this.f.billerId.setValue(this.selectedData.billerId);
    this.f.billerProdCode.setValue(this.selectedData.billerProdCode);
  ...
  populatePostingAccountItems() {
    ...
    this.loading[0] = true;
    this.agentBanking.findBillerProductCategories(request)
      .pipe(first())
      .subscribe(
        response => {
  ...
  onSubmit() {
    ...
    this.loading[0] = true;
    this.agentBanking.maintainBillerProduct(this.form.value)
      .pipe(first())
      .subscribe(
        response => {
          this.loading[0] = false;
