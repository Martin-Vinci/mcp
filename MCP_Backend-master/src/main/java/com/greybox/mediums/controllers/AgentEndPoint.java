package com.greybox.mediums.controllers;

import com.greybox.mediums.entities.Account;
import com.greybox.mediums.entities.AgentsRef;
import com.greybox.mediums.models.TxnResult;
import com.greybox.mediums.services.AgentService;
import com.greybox.mediums.utils.CommonResponse;
import io.swagger.annotations.Api;
import io.swagger.annotations.ApiOperation;
import io.swagger.annotations.Authorization;
import io.swagger.v3.oas.annotations.Operation;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

import static com.greybox.mediums.utils.Logger.logError;
import static com.greybox.mediums.utils.Logger.logInfo;

@RestController
@RequestMapping("/api/v1/agent-banking/agent")
@Api(tags = "Agent Banking Services")
public class AgentEndPoint {

    @Autowired
    private AgentService agentService;

    @PostMapping("/findAgents")
    @ApiOperation(value = "", authorizations = {@Authorization(value = "apiKey")})
    @Operation(summary = "Retrieves all customer addresses in the system")
    public TxnResult findAgents(@RequestBody AgentsRef request) {
        try {
            logInfo(request);
            return agentService.find(request);
        }catch (Exception e){
            logError(e);
            return CommonResponse.getUndefinedError();
        }
    }

    @PostMapping("/findAgentAccounts")
    @ApiOperation(value = "", authorizations = {@Authorization(value = "apiKey")})
    @Operation(summary = "Retrieves all customer addresses in the system")
    public TxnResult findAgentAccounts(@RequestBody Account request) {
        try {
            logInfo(request);
            return agentService.findAgentAccounts(request);
        }catch (Exception e){
            logError(e);
            return CommonResponse.getUndefinedError();
        }
    }

    @PostMapping("/maintainAgents")
    @ApiOperation(value = "", authorizations = {@Authorization(value = "apiKey")})
    @Operation(summary = "Creates a customer address in the system")
    public TxnResult maintainAgents(@RequestBody AgentsRef request) {
        try {
            if (request.getEdit())
                return agentService.update(request);
            else
                return agentService.save(request);
        }catch (Exception e){
            logError(e);
            return CommonResponse.getUndefinedError();
        }
    }
}
